...
# 4. ACTIVE DATA STRUCTURAL ENTITIES MAPPED TO MPEG RECORDINGS
DATA_FLOW_MATRIX = {
    "Smart Speaker": {
        "Paytm": {
            "points": [
            ...
            "audio": "audio/speaker_paytm.mpeg"
        },
        "BharatPe": {
            "points": [
            ...
            "audio": "audio/speaker_bharatpe.mpeg"
        },
        "Google Pay": {
            "points": [
    ...
    "Merchant Lending": {
        ...
        "BharatPe": {
            ...
            "audio": "audio/loan_bharatpe.mpeg"
        },
        "Google Pay": {
            "points": [
    ...
    if current_mod in ["Smart Speaker", "Merchant Lending"]:
        ...
        if selected_dropdown != "Select Competitor...":
            ...
            if st.session_state.pitch_customized:
                ...
                # Streaming and loading local MPEG file assets directly
                try:
                    with open(node["audio"], "rb") as audio_file:
                        st.audio(audio_file.read(), format="audio/mpeg")
                except FileNotFoundError:
                    st.error(f"⚠️ Audio asset missing: Could not find '{node['audio']}' inside your audio folder.")
...
if not st.session_state.selected_module:
    ...
    # Grid column sizing architecture to align labels neatly on a single line
